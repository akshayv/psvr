...
\end{tikzpicture}
\ \\
\centerline{Figure 4.3. Speedup for various datasets excluding Synchronous component.}
\newline\newline
From the graphs in Figure 4.3, we see even if we disregard the sequential portions of the program, none of these speedup curves follow the ideal speedup curve. The deviation from the ideal speedup in these graphs is due to the Synchronization and Communication overheads.
\newline\newline
From the graphs in Figure 4.3, we can also make certain observations regarding the nature of the speedup curve.
\begin{enumerate}[label=(\alph*)]
\item With fewer machines, the effect of the Cholesky Factorization portion is not observed as the time taken for this computation is insignificant when compared to the time taken for the overall computation. As the number of machines used increases, the time taken for computation of other portions decreases (nearly linearly) while the time for executing the Cholesky Factorization remains the same, making the cost of the sequential portion significant.
